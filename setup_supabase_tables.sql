...
-- We can create a custom users table or use the auth.users table directly
...
CREATE POLICY "Parcels are editable by authenticated users" ON public.parcels
...
CREATE POLICY "Properties are editable by authenticated users" ON public.properties
    FOR ALL USING (auth.role() IS NOT NULL);
...
CREATE POLICY "Sales are editable by authenticated users" ON public.sales
    FOR ALL USING (auth.role() IS NOT NULL);
...
CREATE POLICY "Accounts are editable by authenticated users" ON public.accounts
    FOR ALL USING (auth.role() IS NOT NULL);
...
CREATE POLICY "Property images are editable by authenticated users" ON public.property_images
    FOR ALL USING (auth.role() IS NOT NULL);
...
CREATE POLICY "Assessments are editable by authenticated users" ON public.assessments
    FOR ALL USING (auth.role() IS NOT NULL);
